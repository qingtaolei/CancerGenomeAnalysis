...
do
	...
	echo $line | awk '{print "/home/qingtao/qingtao/panelTest/SomaticCall_Delly.sh "$1" "$2" "$3" "$4" "$5}'| awk '{ sub("\r$", ""); print }'  > Scripts/$sample.somatic.delly.sh
	sh Scripts/$sample.somatic.delly.sh
done
