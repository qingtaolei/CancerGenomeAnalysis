...
do
	...
	echo $line | awk '{print "/home/qingtao/qingtao/panelTest/SomaticCall_Mutect2_panel.sh "$1" "$2" "$3" "$4" "$5" "$6}'| awk '{ sub("\r$", ""); print }'  > Scripts/$sample.somatic.mutect2.sh
	sh Scripts/$sample.somatic.mutect2.sh
done
