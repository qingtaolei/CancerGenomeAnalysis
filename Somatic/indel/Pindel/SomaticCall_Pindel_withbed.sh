...
############################################
# Log file 
cd /home/libin/Projects/qtPindel201711_new
#====== bianll======
...
echo $tumor_bam	250 $sample.tumor >> "Results"/$sample.txt
echo $normal_bam 250 $sample.normal >> "Results"/$sample.txt
#./pindel -f hs_ref_GRCh37.fa -p tumor_1222_chr1.txt -c ALL -o tumor_1222
pindel -p "Results"/$sample.txt  -T $nt -f $ref/hg19_nochr.fa -c ALL -o "Results"/$sample.pindel.txt
#pindel -f $ref/hg19_nochr.fa -i  -c ALL -o $sample.pindel

#samtools view $tumor_bam | sam2pindel - "Results"/$sample.tumor.pindel 250 tumor 0 Illumina-PairEnd
#samtools view $normal_bam | sam2pindel - "Results"/$sample.normal.pindel 250 normal 0 Illumina-PairEnd


#pindel -f $ref/hg19_nochr.fa -T $nt -p "Results"/$sample.tumor.pindel  -c ALL -o "Results"/$sample.tumor.pindel.txt
#pindel -f $ref/hg19_nochr.fa -T $nt -p "Results"/$sample.normal.pindel -c ALL -o "Results"/$sample.normal.pindel.txt
...
pindel2vcf -p "Results"/$sample.pindel.txt_${type} -r $ref/hg19_nochr.fa -R hg19  -d 2009 -v "Results"/$sample.$type -e 3
#pindel2vcf -p "Results"/$sample.normal.pindel.txt_${type} -r $ref/hg19_nochr.fa -R hg19  -d 2009 -v "Results"/$sample.normal.$type -e 3
